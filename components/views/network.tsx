  ...
  return <>
    ...
    <section className="section container" aria-labelledby="devices">
      ...
      <div className="card-grid card-grid-3">
        <article className="card card-secondary"><span className="card-icon" aria-hidden="true"><NodeIcon kind="router" size={18} /></span><h3>MikroTik RB5009</h3><p className="mono">RB5009</p><p>{T('Router i brama. Łączy się przez patch panel z głównym switchem; port i medium uplinku nie zostały określone.')}</p><StatusBadge status="planned" locale={locale} label="Plan" /></article>
        <article className="card card-secondary"><span className="card-icon" aria-hidden="true"><NodeIcon kind="switch" size={18} /></span><h3>MikroTik CRS310</h3><p className="mono">CRS310-8G+2S+IN · 8 × 2.5G RJ45 · 2 × SFP+</p><p>{T('Główny switch. Rozprowadza połączenia do pomieszczeń i planowanych punktów dostępowych.')}</p><StatusBadge status="planned" locale={locale} label="Plan" /></article>
        <article className="card card-secondary"><span className="card-icon" aria-hidden="true"><NodeIcon kind="server" size={18} /></span><h3>{T('Serwer HomeIntelCore / NAS')}</h3><p className="mono">SFP+ · Docker Compose</p><p>{T('Usługi i lokalne dane. Osobny link SFP+ wyróżnia ścieżkę do serwera, bez deklarowania niezmierzonej przepustowości.')}</p><StatusBadge status="lab" locale={locale} /></article>
      </div>
    </section>
