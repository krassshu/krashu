...
export function TechTable({ locale = 'pl' }: { locale?: Locale }) {
  return <div className="table-wrap"><table className="tech-table">
    <caption>{t('Komponenty środowiska HomeIntelCore i ich status', locale)}</caption>
    <thead><tr><th scope="col">{t('Komponent', locale)}</th><th scope="col">{t('Rola', locale)}</th><th scope="col">{t('Zakres', locale)}</th><th scope="col">{t('Status', locale)}</th></tr></thead>
